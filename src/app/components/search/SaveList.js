...
const SaveList = ({
  ...
  const [operation, setOperation] = React.useState(objectType === 'feed' ? 'UPDATE_SPECIAL_PAGE' : 'CREATE'); // or 'UPDATE'
  ...
  if (['tipline-inbox', 'suggested-matches', 'feed'].indexOf(objectType) !== -1) {
    let defaultQuery = {};
    let savedQuery = '{}';
