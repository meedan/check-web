...
class SearchQuery extends React.Component {
          ...
          [
            query.projects
              ? query.projects.map((p) => {
                const project = projects.find(pr => pr.dbid === p);
  ...
  subscribe() {
    ...
    pusher.subscribe(team.pusher_channel).bind('tagtext_updated', 'SearchQuery', (data, run) => {
    ...
    pusher.subscribe(team.pusher_channel).bind('project_updated', 'SearchQuery', (data, run) => {
      if (clientSessionId !== data.actor_session_id) {
        if (run) {
          this.props.relay.forceFetch();
          return true;
  ...
  unsubscribe() {
    ...
    pusher.unsubscribe(team.pusher_channel, 'tagtext_updated', 'SearchQuery');
    pusher.unsubscribe(team.pusher_channel, 'project_updated', 'SearchQuery');
  }

  render() {
    const { team, classes, project } = this.props;
    const { statuses } = team.verification_statuses;
    const projects = project
      ? [] // We're querying within one project, so there's no project-select
      : team.projects.edges.map(({ node }) => node).sort((a, b) => a.title.localeCompare(b.title));

    const { currentUser } = this.currentContext();
    const plainTagsTexts = team.tag_texts ?
      team.tag_texts.edges.map(t => t.node.text) : [];

    const pageTitlePrefix = (this.filterIsActive() || this.keywordIsActive())
      ? this.title(statuses, projects)
      : (this.props.title || (project ? project.title : null));
    ...
    return (
      <div>
        ...
        <PageTitle prefix={pageTitlePrefix} team={this.props.team}>
          <Dialog
              ...
              <StyledSearchFiltersSection>
                ...
                {this.showField('project') && projects.length ?
                  <StyledFilterRow>
                    ...
                    {projects.map(({ dbid, title }) => (
                      <StyledFilterChip
                        active={this.projectIsSelected(dbid)}
                        key={dbid}
                        onClick={this.handleProjectClick.bind(this, dbid)}
                        className={bemClass(
                          'search-filter__project-chip',
                          this.projectIsSelected(dbid),
                          '--selected',
                        )}
                      >
                        {title}
...
SearchQuery.contextTypes = {
  store: PropTypes.object,
};

export default createFragmentContainer(
  withStyles(styles)(withPusher(SearchQuery)),
  graphql`
    fragment SearchQuery_team on Team {
      id
      dbid
