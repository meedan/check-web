...
class Task extends Component {
  ...
  renderTaskResponse(responseObj, response, by, byPictures, showEditIcon) {
    ...
    if (
      task.type === "image_upload" &&
      responseObj.image_data &&
      responseObj.image_data.length
    ) {
  ...
  render() {
    ...
    const taskAssignment =
      task.assignments.edges.length > 0 &&
      !response &&
      task.fieldset === "tasks" ? (
        <div className="task__assigned" style={assignmentStyle}>
          <sBox component="small" display="flex">
      ...
      <Box display="flex" alignItems="center">
        ...
        {data.by && isTask ? (
          <Box
            ...
            margin={2}
          >
            <sBox component="span" display="flex">
              <UserAvatars users={byPictures} />
              <Box component="span" lineHeight="24px" px={`${units(1)}`}>
                {response ? (
                  <FormattedMessage
    ...
    return (
      <StyledWordBreakDiv>
        <CustomCard id={`task-${task.dbid}`} className={className.join(" ")}>
          <CardHeader
            disableTypography
            title={taskQuestion}
