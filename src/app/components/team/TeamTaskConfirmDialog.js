...
class TeamTaskConfirmDialog extends React.Component {
  ...
  render() {
    ...
    if (this.props.action === "edit") {
      action = this.props.editLabelOrDescription
        ? "editLabelOrDescription"
        : "edit";
    }
    const deleteConfirmDialogTitle =
      this.props.fieldset === "tasks" ? (
        <FormattedMessage
          id="teamTasks.confirmDeleteTaskTitle"
          defaultMessage="Are you sure you want to delete this task?"
        />
      ) : (
        <FormattedMessage
          id="teamTasks.confirmDeleteMetadataTitle"
          defaultMessage="Are you sure you want to delete this metadata?"
        />
      );
    const editConfirmDialogTitle =
      this.props.fieldset === "tasks" ? (
        <FormattedMessage
          id="teamTasks.confirmEditTaskTitle"
          defaultMessage="Are you sure you want to edit this task?"
        />
      ) : (
        <FormattedMessage
          id="teamTasks.confirmEditMetadataTitle"
          defaultMessage="Are you sure you want to edit this metadata?"
        />
      );
    ...
    if (projects !== null) {
      ...
      projects.forEach((project) => {
        if (
          ...
          selectedProjects.indexOf(project.node.dbid) > -1
        ) {
    ...
    return (
      <Dialog open={this.props.open} onClose={this.props.handleClose}>
        <DialogTitle>{confirmDialogTitle[this.props.action]}</DialogTitle>
        <DialogContent>
          <Message message={this.props.message} />
          <Box m={`${units(2)} 0`}>
            {affectedItems > 0 ? (
              <div>
                <Row>{confirmDialogBlurb[action]}</Row>
                {action !== "edit" ? (
                  <Row>
                    <FormControlLabel
                      control={
