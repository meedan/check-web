  ...
  const onFormSubmit = (e) => {
    ...
    if (checkedTos) {
      const onFailure = (errors) => {
        const error = getErrorMessageForRelayModernProblem(errors);
        const errorMessage = Array.isArray(error) && error[0]?.message ? error[0].message : <GenericUnknownErrorMessage />;
