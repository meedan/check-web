...
const SourceCard = (props) => {
  const { source, team, project_id } = props.source;
      ...
      <CardText className="source-card__content" style={{ display: 'flex', paddingBottom: units(1) }}>
        ...
        >
          <div className="source-card__name">
            <Link
              style={{ font: subheading1, color: black87, fontWeight: 700 }}
              to={sourceUrl}
          ...
          <div className="source-card__description" style={{ paddingTop: units(0.5) }}>
          ...
          <div className="source-card__accounts">
            <ul>
              { source.accounts.edges.map(account => <li key={account.node.id} className="source-card__account-link">
                { MediaUtil.socialIcon(`${account.node.provider}.com`) /* TODO Don't hardcode domain */ }
