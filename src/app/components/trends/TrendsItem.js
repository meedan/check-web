...
const renderQuery = ({ error, props }) => {
  if (!error && props) {
    return <TrendsItemComponent cluster={props.cluster} teams={props.root.current_user.team_users.edges.map(e => e.node.team)} />;
  }
  return null;
};
...
const TrendsItem = props => (
  <QueryRenderer
    ...
    `}
    variables={{
      clusterId: `${props.routeParams.clusterId}`,
    }}
    render={renderQuery}
  />
);
