...
shared_examples 'media' do |type|
  ...
  def create_media_depending_on_type(url = nil, media = 1)
    ...
    when 'DOES_NOT_BELONG_TO_ANY_PROJECT'
      if media == 1
        url.nil? ? api_create_team_project_and_claim_and_redirect_to_media_page({ quote: "Orphan #{Time.now.to_f}" }) : api_create_team_project_and_link_and_redirect_to_media_page({ url: url })
      else
