shared_examples 'team' do
  ...
  it 'should duplicate team', bin1: true do
    ...
    expect(@driver.find_elements(:css, '.switch-teams__joined-team').length == 1)
    ...
    expect(@driver.find_elements(:css, '.switch-teams__joined-team').length == 2)
  ...
  it 'should check user permissions', bin5: true do
    ...
    expect(@driver.find_elements(css: 'div[role=tablist] > button').size).to be == 1
    ...
    expect(@driver.find_elements(:css, '.projects-list__add-filtered-lists').empty?).to be(true)
    ...
    # be able to create a folder and add a filtered list
    wait_for_selector('.project-list__link').click
    wait_for_selector('#search-form')
    expect(@driver.find_elements(:css, '.projects-list__add-filtered-lists').empty?).to be(false)
    expect(@driver.find_elements(:css, '.projects-list__add-folder-or-collection').empty?).to be(false)
