shared_examples 'report' do
  {
    ...
    it "should generate a report from #{provider} video", bin1: true do
      ...
      @driver.navigate.to 'https://paste2.org/'
      wait_for_selector('#container')
      wait_for_selector('#code').send_keys(' ')
      wait_for_selector('#code').send_keys(:control, 'v')
      wait_for_text_change(' ', '#code', :css)
      expect((@driver.find_element(:css, '#code').attribute('value') =~ /medias\.js/).nil?).to be(false)
    end
  end
