...
describe('<ShortTextRespondTask />', () => {
  ...
  it('Cannot submit empty response', () => {
    ...
    expect(task.find('.task__save')).to.have.length(1);
    expect(task.find('.task__save').html()).to.contain('disabled=""');
  });
  ...
  it('Can submit a response', () => {
    ...
    task.instance().canSubmit();
    expect(task.find('.task__save')).to.have.length(1);
    expect(task.find('.task__save').html()).to.not.contain('disabled=""');
  });
